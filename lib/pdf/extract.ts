...
function arcBounds(
  ...
  // 2. Find angles where derivatives = 0
  // 3. Check if those angles are within the arc's sweep
  // This is complex, so we use a sampling approximation
  ...
  for (let i = 1; i < samples; i++) {
    const t = i / samples;
    // Linear interpolation isn't correct for arcs, but provides reasonable bounds
    // For better accuracy, would need full arc parameterization
